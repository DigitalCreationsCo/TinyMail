...
function addDateToHeader(markup: string, date: string) {
  try {
    ...
    if (header) {
      ...
      header.innerHTML = updateInnerHTMLText(
        header.innerHTML,
        (date.toUpperCase() + ' ' + header.textContent).replace(' ', '')
      );
    }
...
function addIntroductionParagraph(
  ...
  try {
    ...
    doc
      ...
      .insertAdjacentElement('beforeend', introductionParagraph);
    // blank line
...
function addArticleToEmail(
  markup: string,
  content: { title: string; body: string; image: string; selector: string },
  textColor: 'light' | 'dark'
): string {
  try {
    const doc = getDocumentFromMarkup(markup);
...
function addLinkToEmail(
  markup: string,
  content: { title: string; link: string; selector: string },
  textColor: 'light' | 'dark'
) {
  try {
    const doc = getDocumentFromMarkup(markup);
...
function addCompanyUpdateToEmail(
  markup: string,
  content: { essay: string; selector: string },
  textColor: 'light' | 'dark'
) {
  try {
    const doc = getDocumentFromMarkup(markup);
    ...
    doc
      .getElementsByTagName(content.selector)[2]
      .insertAdjacentElement('beforeend', essayParagraph);
    doc
      .getElementsByTagName(content.selector)[2]
